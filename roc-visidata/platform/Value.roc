## JSON values: how plugins and Python describe data to each other.
##
## VisiData bridge speaks JSON, so every value that crosses the boundary
## (an event payload, the result of an expression, the arguments to a function
## call) arrives as a `Value` value and leaves as one.
Value := [
    ## `null`, and what Python sends for `v:null`.
    Null,
    ## `true` / `false`.
    Bool(Bool),
    ## A whole number. a whole number. Keep line numbers and buffer
    ## numbers as `Int`: Python refuses a `Float` where it wants a `Number`.
    Int(I64),
    ## A number with a fractional part. VisiData's `Float` type.
    Float(F64),
    ## A string. VisiData's `String` type.
    Text(Str),
    ## A list, in order. VisiData's `List` type.
    Array(List(Value)),
    ## An object, in the order its fields were written. VisiData's `Dict` type.
    ...
    ## number, because Python writes `2.0` for some numbers that are conceptually
    ## integers.
    ...
    ## The boolean in a `Bool`. Python has no boolean type of its own, so it sends
